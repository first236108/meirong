...
class Auth
{
     ...
     * @param int $type int         执行check的模式
     * @param string $mode 当前访问的url
    ...
    protected function getAuthList($uid, $type)
    {
        ...
        //读取用户组所有权限规则
        $rules = Db::table($this->_config['AUTH_RULE'])->where('id', 'in', $ids)->where('status', 1)->field('condition,name')->select();
        //循环规则，判断结果。
        $authList = [];   //
